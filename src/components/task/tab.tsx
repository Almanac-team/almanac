...
const TabContext = React.createContext<TabContextValue>({activeValue: undefined, onChange: undefined});
...
export function Tab({value, children}: TabProps) {
    const {activeValue, onChange} = React.useContext<TabContextValue>(TabContext);

    return (
        <li className="mr-2 w-full" onClick={() => {if (onChange != undefined && value != undefined) {onChange(value)}}}>
