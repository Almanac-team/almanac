...
const generatedEventsRouter = createTRPCRouter({
    createGeneratedEvents: protectedProcedure.input(z.object({
        ...
        const tasks = activities.filter((activity) => isTask(activity)) as ActivitySetting<TaskSetting>[];
        const events = activities.filter((activity) => isEvent(activity)) as ActivitySetting<EventSetting>[];

        const generatedEvents = generateEvents(tasks, events, input.firstDayOfWeek);

        return (await ctx.prisma.generatedEvent.createMany({
    ...
    getGeneratedEvents: protectedProcedure.query(async ({ctx}): Promise<GeneratedEvent[]> => {
        const userId: string = ctx?.session?.user?.id;
        return (await ctx.prisma.generatedEvent.findMany({
            where: {
                activity: {
                    category: {
                        userId: userId
